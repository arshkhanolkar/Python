...
class StudyTrackerApp:
    ...
    def init_pages(self):
        ...
        # Graphs Page (new)
        graphs_frame = ttk.Frame(self.container)
    ...
    def build_main_ui(self, frame):
        ...
        self.goal_var = tk.StringVar(value=str(DEFAULT_DAILY_GOAL))
        ...
        # Modified graph buttons (only change to home screen UI)
        ttk.Button(frame, text="Quick Graph", command=self.show_graph,
                  style="Graph.TButton").grid(row=7, column=0, pady=(10, 0))
        ttk.Button(frame, text="More Visualizations →", 
                  command=lambda: self.show_page("GraphsPage"),
                  style="Accent.TButton").grid(row=8, column=0, pady=(0, 10))
    ...
    def build_graphs_ui(self, frame):
        ...
        # Back button
        ttk.Button(frame, text="← Back to Main", 
                  command=lambda: self.show_page("MainPage"),
                  style="Primary.TButton").grid(row=0, column=0, sticky="nw", padx=10, pady=10)
        
        # Title
        ttk.Label(frame, text="Study Visualizations", 
                 font=("Helvetica", 18, "bold")).grid(row=1, column=0, pady=20)
        ...
        for i, (title, desc, func) in enumerate(graph_options, start=2):
            ttk.Button(frame, text=title,
                      command=lambda f=func: f(self.logger.get_weekly_data(), self.daily_goal),
                      style="Graph.TButton").grid(row=i, column=0, pady=5, padx=20, sticky='ew')
            ttk.Label(frame, text=desc, wraplength=400).grid(row=i, column=1, sticky='w', padx=10)
    ...
    def apply_theme(self):
        ...
        style.configure("Graph.TButton", background="#9C27B0", foreground="white")
