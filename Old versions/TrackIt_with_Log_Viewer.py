    ...
    try:
        ...
        with open(CSV_FILE, newline='') as f:
            ...
            for row in reader:
                ...
                if date not in hourly_by_day:
                    hourly_by_day[date] = [0] * 24
                hourly_by_day[date][hour] += minutes
        ...
        for h in range(24):
            ...
            for day in last_7_dates:
                mins = hourly_by_day.get(day, [0] * 24)[h]
                total += mins
                count += 1
            hourly_totals[h] = total / count if count else 0
        ...
        plt.title("Hourly Productivity — Last 7 Days")
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.tight_layout()
